...
def start_training(device_name="cpu", lr=0.1, epochs=10, batch_size=5, num_workers=2, lr_gamma=0.6, data_path="./data"):
    ...
    train_set = MNIST(data_path, train=True, download=True, transform=transforms.ToTensor())
    train_dataloader = DataLoader(train_set, batch_size=batch_size, num_workers=num_workers, shuffle=True)

    valid_set = MNIST(data_path, train=False, download=True, transform=transforms.ToTensor())
